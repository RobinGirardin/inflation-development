...
Nonetheless, we further simplify the situation and only look at the mean inflation per wealth class. As such, for simplicity, we assume independance of observation and test wether the wealth class has an influence on the inflation rate. A common test for this scenario would be an analysis of variance (ANOVA) or non-parametric tests.
...
```{r}
prep_join <- join_labeled %>% 
    select(wealth_class, inflation_pct, country) %>% 
    drop_na()

kruskal.test(inflation_pct ~ wealth_class, data = prep_join)
```
